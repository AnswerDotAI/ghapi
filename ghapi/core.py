...
# %% auto #0
__all__ = ['GH_HOST', 'pspec', 'img_md_pat', 'EMPTY_TREE_SHA', 'MAXLEN', 'print_summary', 'GhTransport', 'GhSyncTransport',
           'gh_patch', 'GhApi', 'call_gh', 'date2gh', 'gh2date', 'GhRows', 'create_release', 'delete_release',
           'list_tags', 'list_branches', 'create_branch_empty', 'delete_tag', 'delete_branch', 'get_branch',
           'commit_tree', 'list_files', 'get_content', 'create_or_update_file', 'create_file', 'delete_file',
           'update_contents', 'enable_pages', 'read_issue', 'read_pr', 'pr_file_diff', 'PullRow', 'PullRows',
           'list_prs', 'issue_template', 'issue_body', 'CheckRun', 'CommitStatus', 'check_status', 'pr_status',
           'failed_step_log', 'dep_key', 'local_dep_graph', 'dep_closure', 'dep_order', 'dep_dependents', 'dep_graph',
           'APIError']
...
class GhApi(OpenAPIClient):
    ...
    def __init__(self,
        owner=None, # Default repository owner; endpoint calls can override it
        repo=None, # Default repository name; endpoint calls can override it
        token=None, # Personal access token; otherwise GITHUB_TOKEN
        jwt_token=None, # JWT; otherwise GITHUB_JWT_TOKEN; takes precedence over token
        debug=None, # Request callback (method, url, kwargs); print_summary omits authorization
        limit_cb=None, # Callback (remaining, quota) when rate-limit headers change
        gh_host=None, # API base URL; otherwise GH_HOST
        authenticate=True, # False skips credentials; missing credentials otherwise warn and remain unauthenticated
        timeout=60.0, # Request timeout in seconds
        sync=False, # Generated endpoints block instead of returning awaitables; async convenience methods still need an async client
        **kwargs # Additional default route/query parameters
    ):
        "Construct the generated API locally; no request is made until an operation is called"
...
def date2gh(dt:datetime)->str:
    ...
    return f'{dt.replace(tzinfo=None, microsecond=0).isoformat()}Z'
...
async def delete_file(self:GhApi, path, message, committer, author, sha=None, branch=None):
    if not branch: branch = (await self.repos.get())['default_branch']
    if sha is None: sha = (await self.list_files(branch))[path].sha
...
async def read_issue(self:GhApi, issue_number:int):
    """Fetch an issue or PR as structured data with a compact display.

    Fields include `title`, `body`, `is_pr`, and general `comments`. PRs also have the full `diff`, inline `review_comments`, and `reviews`. Display the result bare for the summary; inspect those fields for detail. General PR comments use the Issues API, not the Pulls review API.
    """
...
class PullRow(AttrDict):
    ...
    def line(self, maxlen=MAXLEN): return f'#{self.number}  {_age(self.updated_at)}  {_pr_mark(self)}{truncstr(self.title, maxlen)}'
    __repr__ = line

# %% ../nbs/00_core.ipynb #21b48014
class PullRows(GhRows):
    "Pull request rows, displayed with titles truncated to `maxlen`"
    def __init__(self, items, maxlen=MAXLEN):
...
async def list_prs(
    ...
    maxlen=MAXLEN, # Title characters shown per row
...
# %% ../nbs/00_core.ipynb #c74057c8
def _parse_section(b):
    attrs = b['attributes']
    options = [o['label'] if isinstance(o, dict) else o for o in attrs.get('options', [])]
    return dict(label=attrs['label'], type=b['type'], required=bool(nested_idx(b, 'validations', 'required')), options=options)
...
def _parse_tmpl(name, txt):
    ...
    secs = [_parse_section(b) for b in d.get('body', []) if b['type']!='markdown']
    return dict2obj(dict(name=name, title=d.get('name'), description=d.get('description'), sections=secs))

async def _load_tmpl(api, f, **kw):
    txt = (await api.get_content(f.path, **kw)).decode()
    return _parse_tmpl(f.name, txt)
...
async def issue_template(self:GhApi):
    ...
    for repo,path in ((None,'.github/ISSUE_TEMPLATE'), ('.github','ISSUE_TEMPLATE'), ('.github','.github/ISSUE_TEMPLATE')):
        ...
        try: fs = await self.repos.get_content(path=path, **kw)
            ...
            if e.status_code != 404: raise
            continue
        return L([await _load_tmpl(self, f, **kw) for f in fs
            if f.name.endswith(('.md','.yml','.yaml')) and f.name != 'config.yml'])
    return L()
...
async def check_status(self:GhApi, ref:str):
    ...
    Display the result bare for the check-run verdict and all run/status rows. `.state` comes only from legacy commit statuses and can be pending even when every Actions check passed. `.check_runs` and `.statuses` retain the structured rows.
    """
