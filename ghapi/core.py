...
# %% auto #0
__all__ = ['GH_HOST', 'pspec', 'img_md_pat', 'EMPTY_TREE_SHA', 'GhTransport', 'print_summary', 'GhApi', 'call_gh', 'date2gh',
           'gh2date', 'read_pr', 'APIError']

# %% ../nbs/00_core.ipynb #5b5cba7b
from fastcore.all import *
from fastspec.spec import SpecParser
from fastspec.oapi import OpenAPIClient, OpFunc, _build_groups
...
import os, shutil, tempfile, subprocess, fnmatch, asyncio
...
class GhTransport(AsyncTransport):
    ...
    async def request(self, method, url, *, raw=False, **kwargs):
        ...
        resp = await super().request(method, url, raw=True, **kwargs)
...
class GhApi(OpenAPIClient):
    ...
    def __init__(self, owner=None, repo=None, token=None, jwt_token=None, debug=None, limit_cb=None, gh_host=None,
                 authenticate=True, timeout=60.0, **kwargs):
        ...
        self.transport = GhTransport(debug=debug, limit_cb=limit_cb, timeout=timeout, base_headers=self.headers)
        self.ops = [OpFunc(o, self.transport, self.gh_host, defaults=kwargs) for o in pspec.ops]
    ...
    async def __call__(self, path:str, verb:str=None, headers:dict=None, route:dict=None, query:dict=None, data=None):
        ...
        return await self.transport.request(verb, path, headers=headers, params=query, **kw)
...
def call_gh(op:str, *args, token=None, **kwargs):
    "Call one GitHub operation `op` (e.g. `'repos.get'`) synchronously; a fallback for code that can't easily be async"
    f = GhApi(token=token)
...
async def get_repo_contents(self:GhApi, owner, repo, branch='main', **kwargs):
    repo_files = await self.get_repo_files(owner, repo, **kwargs)
    return L(await asyncio.gather(*[self.get_file_content(p, owner=owner, repo=repo, branch=branch)
                                    for p in repo_files.attrgot("path")]))
