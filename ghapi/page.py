...
# %% auto #0
__all__ = ['paged', 'parse_link_hdr', 'pages', 'sync_paged', 'sync_pages', 'gh_notifs']
...
async def gh_notifs(days=5, reasons=('mention', 'review_requested', 'author', 'assign'), include_read=False, per_page=100):
    ...
    notifs = L([o async for o in paged(GhApi().activity.list_notifications_for_authenticated_user, all=include_read, since=since, per_page=per_page)]).concat()
    if reasons: notifs = notifs.filter(lambda n: n.reason in reasons)
    lines = []
    for n in notifs:
        repo, kind = n.repository.full_name, n.subject.type
        num = n.subject.url.split('/')[-1] if n.subject.url else '?'
        url = f"https://github.com/{repo}/pull/{num}" if kind == 'PullRequest' else f"https://github.com/{repo}/issues/{num}" if kind == 'Issue' else n.subject.url
        lines.append(f"- [{kind} #{num}]({url}) {repo} — {n.subject.title} ({n.reason})")
    scope = f"past {days} days" if days else "all available"
    reason_txt = ", ".join(reasons) if reasons else "all reasons"
    return f"**{len(lines)} notifications** ({scope}; {reason_txt})\n\n" + "\n".join(lines)
