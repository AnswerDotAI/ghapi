...
> directly in Jupyter and modern REPLs; in scripts, use `asyncio.run`.
...
[`ghapi`](https://ghapi.fast.ai/cli.html#ghapi) provides Python and
command-line access to the [GitHub REST
API](https://docs.github.com/rest). It builds the Python API dynamically
from a compact, pre-parsed copy of GitHub’s [OpenAPI
spec](https://docs.github.com/rest/overview/openapi-description).
...
Read an endpoint’s full documentation on the generated object. Its
display includes parameter descriptions, bound defaults, request
controls, and async usage. With pyskills, use `doc(api.git.get_ref)` for
the same documentation, or `xdir(api.git, 'ref')` to search names.
Construction and discovery send no requests:
...
Jupyter provides full tab completion and parameter lists for every
endpoint. Endpoints are called as standard Python methods:
...
To use authenticated operations (other than when running through GitHub
...
here](https://github.com/settings/tokens/new) to create it, choosing the
scopes you need (“repo”, “gist”, “notifications”, and “workflow” cover
most uses). Save it as an environment variable named `GITHUB_TOKEN`,
e.g. by adding this to your `.bashrc` or `.zshrc`:

    export GITHUB_TOKEN=xxx

[`GhApi`](https://ghapi.fast.ai/core.html#ghapi) uses that variable
automatically, or you can pass `token=` explicitly.

As well as `token`, you can pass any parameters you want auto-inserted
into relevant methods, such as `owner` and `repo`:
...
The [GhApi details](https://ghapi.fast.ai/core.html) page covers helpers
for common tasks:

- [`paged`](https://ghapi.fast.ai/page.html#paged) and
  [`pages`](https://ghapi.fast.ai/page.html#pages) fetch results from
  paginated endpoints.
- [`read_issue`](https://ghapi.fast.ai/core.html#read_issue) and
  [`read_pr`](https://ghapi.fast.ai/core.html#read_pr) collect issue or
  PR details, including comments, diffs and reviews as applicable.
- [`dep_graph`](https://ghapi.fast.ai/core.html#dep_graph) and
  [`dep_order`](https://ghapi.fast.ai/core.html#dep_order) find
  repository dependencies and put repositories in dependency order.
- Other helpers manage gists, releases, branches and repository files.

For CI, [`check_status`](https://ghapi.fast.ai/core.html#check_status)
combines commit statuses and check runs.
[`failed_step_log`](https://ghapi.fast.ai/core.html#failed_step_log)
retrieves logs from a failed step. Here’s the status of a branch:
...
Use the [GraphQL client](https://ghapi.fast.ai/graphql.html) to read
several repositories in one query instead of making a REST call for
each. Build query fragments by attribute chaining, with schema checks as
you go. `batch` groups them into parallel requests.

Here we fetch several repositories’ head commits. The GraphQL docs also
cover interactive schema discovery, cursor pagination and raw queries:
